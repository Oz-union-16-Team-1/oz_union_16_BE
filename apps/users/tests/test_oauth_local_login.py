...
class KakaoLoginViewTest(TestCase):
    """GET /api/v1/accounts/social-login/kakao/login/local → 카카오 인증 페이지로 302 redirect"""

    def setUp(self) -> None:
        self.client = APIClient()
        self.url = reverse("kakao-local-login")
...
class KakaoCallbackViewTest(TestCase):
    """GET /api/v1/accounts/social-login/kakao/callback/local"""
...
class NaverLoginViewTest(TestCase):
    ...
    def setUp(self) -> None:
        self.client = APIClient()
        self.url = reverse("naver-local-login")
...
class GoogleLoginViewTest(TestCase):
    ...
    def setUp(self) -> None:
        self.client = APIClient()
        self.url = reverse("google-local-login")
...
class NicknameDeduplicationTest(TestCase):
    ...
    def test_duplicate_nickname_gets_suffix(self, mock_post, mock_get) -> None:
        ...
        self.client = APIClient()
        response = self.client.get(reverse("kakao-local-callback"), {"code": "test_code"})

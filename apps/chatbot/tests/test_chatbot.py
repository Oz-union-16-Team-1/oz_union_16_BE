...
class ChatbotAPITest(TestCase):
    ...
    def test_account_knowledge_includes_id_and_password_help(self) -> None:
        ...
        self.assertEqual(
            ...
            "비밀번호를 잊은 경우 비밀번호 찾기 또는 재설정 기능을 통해 새 비밀번호로 변경할 수 있습니다. 소셜 로그인 사용자는 각 소셜 서비스에서 비밀번호를 관리해 주세요.",
        )
        self.assertEqual(
            build_answer("패스워드를 변경하고 싶어"),
            "소셜 로그인 사용자는 사이트에서 비밀번호를 변경할 수 없습니다. 일반 회원은 마이페이지에서 비밀번호를 변경할 수 있습니다.",
        )
    ...
    def test_chatbot_filter_rules_cover_common_follow_up_questions(self) -> None:
        cases = {
            ...
            "비밀번호 변경하고 싶어요": "소셜 로그인 사용자는 사이트에서 비밀번호를 변경할 수 없습니다. 일반 회원은 마이페이지에서 비밀번호를 변경할 수 있습니다.",
    ...
    def test_match_questions_use_match_answers(self) -> None:
        match_intro_answer = (
            "매칭 추천은 설문 없이 장르를 먼저 선택해 빠르게 게임을 추천받는 기능입니다. "
            "액션/격투, 어드벤처/플랫폼, RPG/스토리, 전략/시뮬, 스포츠/레이싱, 두뇌/퍼즐, "
            "슈팅, 음악/리듬 중 하나를 선택해 시작할 수 있습니다."
        )
        cases = {
            "매칭은 어떻게 해요": match_intro_answer,
            "매칭 추천은 뭐야": match_intro_answer,
            "장르 매칭은 뭐야": match_intro_answer,
            "매칭 후보는 어떻게 나와": "매칭 후보 게임은 선택한 장르를 기준으로 평가할 게임을 최대 5개까지 제공합니다. 이미 좋아요한 게임이나 블랙리스트 게임, 설명이나 트레일러가 부족한 게임은 후보에서 제외됩니다.",
            "매칭 결과는 어디서 봐": "매칭 결과는 사용자가 선택한 장르와 별점 평가를 바탕으로 높은 점수를 준 게임과 유사한 게임을 우선 추천합니다. 결과는 개인화 점수 기준으로 정렬됩니다.",
        }
